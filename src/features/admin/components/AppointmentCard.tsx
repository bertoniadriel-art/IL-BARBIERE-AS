...
export function AppointmentCard({ app, currencyFormatter, onMutated }: AppointmentCardProps) {
    ...
    return (
        <>
            <div
                className={`p-4 rounded-xl border mb-3 transition-all ${localStatus === "attended"
            ...
            >
                <div className="flex justify-between items-start mb-2">
                    ...
                    <span
                        className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-tighter ${localStatus === "attended"
            ...
            {isMoveModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
                    <div className="w-full max-w-sm mx-4 glass-card border-neon-cyan/40 p-6 relative animate-in zoom-in-95 duration-300">
                        ...
                        {moveError && (
                            <p className="mt-3 text-xs text-red-400">{moveError}</p>
                        )}
                        <button
                            type="button"
                            disabled={isMoving}
