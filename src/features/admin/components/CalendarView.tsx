...
export function CalendarView({ barber, onMutated }: CalendarViewProps) {
  ...
  const {
    ...
    currencyFormatter,
  } = useDashboardMetrics(appointments);
    ...
    return (
      <div className='text-center py-12 p-8 glass-card border-yellow-500/20'>
        ...
        <p className='text-white/40 text-sm'>
          Configura las variables de entorno de Supabase para ver la agenda real.
        </p>
      </div>
    );
  }
      ...
      <div className='mb-8 flex flex-col gap-4 md:flex-row md:items-stretch md:justify-between'>
        <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 flex-1'>
          <div className='glass-card p-4 border-neon-cyan/30 flex flex-col justify-between'>
            ...
            <p className='text-[10px] text-white/30 mt-2 uppercase'>
          ...
          <div className='glass-card p-4 border-white/10 flex flex-col justify-between'>
            ...
            <div className='space-y-1 text-xs'>
              {topClients.length === 0 && <p className='text-white/30'>Sin datos este mes.</p>}
      ...
      {isOverbookModalOpen && (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm'>
          <div className='w-full max-w-md mx-4 glass-card border-red-500/40 p-6 relative animate-in zoom-in-95 duration-300'>
            ...
            {overbookError && (
              <p className='mt-4 text-xs text-red-400 font-bold text-center'>{overbookError}</p>
            )}
            <button
              type='button'
